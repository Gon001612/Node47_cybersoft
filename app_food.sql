...
user_id INT, FOREIGN KEY (user_id) REFERENCES users(user_id),
food_id INT, FOREIGN KEY (food_id) REFERENCES foods(food_id),
amount INT,
code VARCHAR(255),
...
SELECT COUNT(u.user_id) as 'so like', u.user_id, u.full_name FROM users u 
...
SELECT COUNT(r.res_id) as 'so like', r.res_id, r.res_name from restaurant r
...
SELECT COUNT(u.user_id) as 'luot order', u.user_id, u.full_name FROM users u
INNER JOIN orders o on o.user_id = u.user_id
GROUP BY u.user_id, u.full_name
ORDER BY COUNT(u.user_id) DESC
LIMIT 1

-- Người không hoạt động (Không đặt hàng, không like, không đánh giá nhà hàng)
